...
int main(int argc, char* argv[]) {
	...
	int lockID = acquireBlockLocks(SERVICE_NAME, SERVICE_PORT, fileName,
                                  3, offsets, lengths);
	printf("lockservertest: Locks acquired; lock ID=%d\n", lockID);
	printf("lockservertest: Sleeping to emulate work...\n");
	sleep(1);
	acquireMoreBlockLocks(SERVICE_NAME, SERVICE_PORT, fileName, lockID,
                         3, offsets2, lengths2);
	printf("lockservertest: More locks acquired\n");
	printf("lockservertest: Sleeping to emulate work...\n");
	sleep(1);
	releaseBlockLocks(SERVICE_NAME, SERVICE_PORT, fileName, lockID);
