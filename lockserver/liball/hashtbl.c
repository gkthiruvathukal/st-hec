...
/* moved dispose functions to header - 03/25/2005 pma */
#else
...
/* changed to PUBLIC - 03/25/2005 pma */
PUBLIC 
#ifdef __ANSI_C__
...
PUBLIC void HashTableApply(HashTable t, ApplyFunctionGeneric f,
						  void* args[]) {
	int i;
	for (i = 0; i < t->size; ++i) {
		if (t->bucket[i]) {
			AVLTreeInorderApply2(t->bucket[i], f, args);
		}
	}
}
